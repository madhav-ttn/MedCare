...
const slotModel = {
  ...
  getSlotByDoctorId: async (doctor_id: number, date: string) => {
    try {
      const result = await pool.query(
        "SELECT doc.name,doc.location,doc_s.* from doctor_slots as doc_s JOIN doctors as doc on doc_s.doctor_id = doc.id  where doctor_id=$1 and date=$2",
        [doctor_id, date]
      );
      return {
        success: true,
  ...
  deleteSlotById: async (slot_id: number, doctor_id: number) => {
    try {
      const result = await pool.query(
        "DELETE from doctor_slots where id=$1 AND doctor_id=$2",
        [slot_id, doctor_id]
      );
      return {
        success: true,
